...
if($_POST['act'] == 'address'){
	...
	$hrefs[] = 'draw.php';
	$hrefs[] = '/';
	show_message('收货地址设置成功',$links,$hrefs);
	...
	$res['content'] = $smarty->fetch('draw_list.dwt');
	$res['error'] = 0;
	...
	if(empty($draw)){
		$links[] = '返回首页';
		$hrefs[] = '/';
		show_message('暂无活动进行',$links,$hrefs);
	}
	$draw_id =  $draw['did'];
	$lotteryTitle =  $draw['draw_title'];

	assign_template($_SESSION[user_name]);
