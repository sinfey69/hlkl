...
function changeAddress(address_id)
{
  ...
  Ajax.call('flow.php?step=change_default_address', 'address_id=' + address_id, orderAddressResponse, 'GET', 'JSON');
}
/* *
 * 改变默认收货地址
 */
function changeDefaultAddress(address_id)
{
  if (address_id < 1)
  {
    return;
  }
  Ajax.call('flow.php?step=change_address', 'address_id=' + address_id, orderAddressResponse, 'GET', 'JSON');
}
/* *
 * 回调函数
 */
function orderAddressResponse(result)
{
  if (result.error)
  {
   return false;
  ...
  try
  {
    //var layer = document.getElementById("ECS_ADDRESSLIST");
    //layer.innerHTML = (typeof result == "object") ? result.content : result;
    $('#ECS_ADDRESSLIST').html((typeof result == "object") ? result.content : result);
    var layer = document.getElementById("ECS_CONFIRMADDRESS");
    layer.innerHTML = (typeof result == "object") ? result.confirm_address : result;
  }
  catch (ex) { }
}