...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer','send_code');
...
elseif ($action == 'is_registered')
{
	...
	else
	{
	   ...
	   {
		   echo 'true';//.json_encode(check_ccs($username));
       }
       else
	   {
           echo 'no_mem';//.json_encode(check_ccs($username));
	   }
    }
}
...
elseif ($action == 'login')
{
    if (empty($back_act))
    {
        ...
        {
           // $back_act = 'user.php';
		   $back_act = 'index.php'; //2014-04-13 Update by Jole
        }
...
//var_dump(array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act,'index.php'));
//die();

      //  show_message($_LANG['login_success'] . $ucdata , array($_LANG['back_up_page'], $_LANG['profile_lnk']), array($back_act,'index.php'), 'info');
		  show_message($_LANG['login_success'] . $ucdata , array($_LANG['back_up_page'], $_LANG['profile_lnk']), 'index.php', 'info');
    }
    else
    {
        $_SESSION['login_fail'] ++ ;
...
elseif ($action == 'act_edit_profile')
{
    ...
    /* 2014-04-25 Note by Jole 取消会员邮箱注册
    if (!is_email($email))
    {
...
elseif ($action == 'get_password')
{
    ...
    if (isset($_POST['sms_code']) && isset($_POST['username']))
    {
        $code = trim($_POST['sms_code']);
        $username  = $_POST['username'];

        /* 判断链接的合法性 */
        $user_info = $user->get_profile_by_name($username);//$user->get_profile_by_id($uid);

        if (empty($user_info) || ($user_info && md5($username .$_CFG['hash_code']. $code) != $_SESSION['mobile_code']))
        {
            show_message('验证码错误');
        }
		unset($_SESSION['mobile_code']);
        $smarty->assign('uid',    $user_info['user_id']);
        $smarty->assign('code',   md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']));
...
elseif ($action == 'affirm_received')
{
    ...
    if (affirm_received($order_id, $user_id))
    {
		...
		echo SDT_PLACE;
		$sql=$_CFG['use_storage'].$_CFG['stock_dec_time'].SDT_PLACE;
		file_put_contents("bill/lastsql.txt",$sql."\r\n",FILE_APPEND);
		if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
		{
			change_order_goods_storage($order_id, true, SDT_SHIP);
		}
