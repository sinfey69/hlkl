...
else
{
    ...
    $sql   = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('goods') . " AS g, " .
			$GLOBALS['ecs']->table('exchange_goods') . " AS eg " .
			"WHERE eg.goods_id = g.goods_id AND eg.is_exchange = 1 AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
    ...
    /*$sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "ORDER BY $sort $order";*/
	$sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
			"g.goods_thumb, g.goods_img,eg.exchange_integral, eg.goods_price ".
		"FROM " .$ecs->table('exchange_goods'). " AS eg ".
		"LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ".
				"ON g.goods_id = eg.goods_id ".
		"WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND eg.is_exchange = 1 $attr_in ".
			"AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
		"ORDER BY $sort $order";
	...
	$url_format = "search.php?keywords=" . urlencode(stripslashes($_REQUEST['keywords']));
	/*
    if (!empty($intromode))
    {
