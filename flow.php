...
if ($_REQUEST['step'] == 'add_to_cart')
{
     ...
     */
    //取出商品的积分
 	$sql = "SELECT exchange_integral as integral FROM ".$GLOBALS['ecs']->table('exchange_goods')." WHERE goods_id=".$goods->goods_id;
	
	file_put_contents("bill/lastsql.txt",$sql."\r\n",FILE_APPEND);
    $jf = $GLOBALS['db']->getOne($sql);
  	//当前用户的积分  
     $ujf = get_can_use_winning($_SESSION['user_name']);
     if($ujf < $jf){
     	//$result['error'] = 5;
     	//$result['message'] = "抱歉，您的积分不够！";
     	//die($json->encode($result));
     	//exit;
     }
    ...
    if (empty($goods->spec) AND empty($goods->quick))
    {
        $sql = "SELECT a.attr_id, a.attr_name, a.attr_type, ".
            "g.goods_attr_id, g.attr_value, eg.goods_price as attr_price " .
    ...
    */
    ...
    else
    {
        ...
        if ($_SESSION['user_id'] > 0)
        {
            ...
            $consignee['address_id'] = save_consignee($consignee);
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        ...
        "AND parent_id = 0 AND now_buy = 1 AND is_gift = 0 AND rec_type = '$flow_type'";
    ...
    if ($db->getOne($sql) == 0)
    {
		...
		$links[] = '返回购物车';
		$hrefs[] = 'binfen.php';
		$hrefs[] = 'flow.php?step=cart';
		show_message('您当前没有需结算的商品',$links,$hrefs,'warning');
    }
    ...
    if ($unpay_count > 0) {
		show_message('对不起，您有 '.$unpay_count.' 笔尚未支付的订单，请先处理该订单。');
    }
   ...
   //$smarty->assign('consignee', $consignee);
    ...
    /* 对商品信息赋值 */
	//echo $flow_type;
    $cart_goods = cart_integral_goods($flow_type); // 取得商品列表，计算合计
	//var_dump($cart_goods);
    ...
    if ($total['exchange_integral'] > $user_points ){
        show_message('对不起，您现有的积分值不够兑换本订单以及待兑换订单。');
    }

    $smarty->assign('total', $total);
    //$smarty->assign('shopping_money', sprintf($_LANG['shopping_money'], $total['formated_goods_price']));
	//var_dump($total['exchange_integral']);
	$smarty->assign('shopping_money', sprintf($_LANG['shopping_money'], $total['exchange_integral'].$_CFG['integral_name'].'+'.$total['formated_goods_price']));  //2014-04-28 Update by Jole
    ...
    $default_address_id = $GLOBALS['db']->getOne($sql);
    ...
    foreach ($consignee_list as $key => $val) {
        if (check_consignee_info($val, $flow_type)){
            if($val['address_id'] == $_SESSION['flow_consignee']['address_id']){
                $consignee_list[$key]['is_checked'] = 1;
            }
            if($val['address_id'] == $default_address_id){
                $consignee_list[$key]['is_default'] = 1;
            }
            $consignee_list[$key]['province'] = get_regionsName($val['province']);
            $consignee_list[$key]['city'] = get_regionsName($val['city']);
            $consignee_list[$key]['district'] = get_regionsName($val['district']);
        }else{
    ...
    $consignee['province'] = get_regionsName($_SESSION['flow_consignee']['province']);
    $consignee['city'] = get_regionsName($_SESSION['flow_consignee']['city']);
    $consignee['district'] = get_regionsName($_SESSION['flow_consignee']['district']);
    $smarty->assign('consignee', $consignee);
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
        ...
        $smarty->assign('pay_name', $payment_info['pay_name']);
        /* 保存 session */
        $_SESSION['flow_order'] = $order;
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        " WHERE session_id = '" . SESS_ID . "' " .
        "AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type' AND now_buy = 1";
    if ($db->getOne($sql) == 0)
    {
   ...
   /* 判断是不是实体商品 */
	...
	$user_info   = get_user_info($_SESSION['user_id']);
    $user_points = $user_info['pay_points']; // 用户的积分总数
	$sql="SELECT sum(integral) FROM " .$ecs->table('order_info'). " WHERE order_status=0 and  user_id = '".$_SESSION['user_id']."'";
	//echo $sql;
	$record_count = $db->getOne($sql);
	if ($record_count >= $user_points ){
		show_message($_LANG['eg_error_integral2'], array($_LANG['back_up_page']), array($back_act), 'error');
	}
    ...
    if ($order['order_amount'] <= 0)
    {
        ...
        if($score >= $total['exchange_integral']){
			$data = update_Winning($_SESSION['user_name'], $total['exchange_integral'],$order['order_sn']);
            if($data == 'true'){
                $order['is_deduct'] = 1;
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
            ...
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id".
            " FROM " .$ecs->table('cart') .
            " WHERE session_id = '".SESS_ID."' AND rec_type = '$flow_type' AND now_buy = 1";
    $db->query($sql);
    /* 修改拍卖活动状态 */
    ...
    /* 如果使用库存，且下订单时减库存，则减少库存 */
		/*echo $_CFG['use_storage'];
		echo $_CFG['stock_dec_time'];
		echo SDT_PLACE;
		*/
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
    {
    ...
    if ($order['order_amount'] <= 0)
    {
        $sql = "SELECT goods_id, goods_name, goods_number AS num FROM ".
               $GLOBALS['ecs']->table('cart') .
                ...
                " AND session_id = '".SESS_ID."' AND rec_type = '$flow_type' AND now_buy = 1";
     ...
     */
	 /*
    $num = intval($_POST['integral']);
    if($num){
		$score = get_can_use_winning($_SESSION['user_name']);
		if($score > 0){
			update_Winning($_SESSION['user_name'], $num);
		}
    }
	*/
	...
	/*
		...
		if($order['integral']){
			$score = get_can_use_winning($_SESSION['user_name']);
			if(($score-$order['integral']) > 0){
				update_Winning($_SESSION['user_name'], $order['integral']);
			}
		}
	}
	*/
...
/*------------------------------------------------------ */
//-- 更新购物车
//--Modify By SouL_Z 2014-7-30 16:03
/*------------------------------------------------------ */
...
elseif ($_REQUEST['step'] == 'update_cart')
{
	...
	if(!empty($id)){
		...
		if(isset($_POST['num'])){
			...
			if($num > 0){
                if ($GLOBALS['_CFG']['use_storage'] == 1){
                    $rec_id = implode(',',$id);
                    $sql = "SELECT goods_id,goods_attr_id,goods_number FROM  " .$GLOBALS['ecs']->table('cart')." WHERE rec_id='".$rec_id."' AND session_id='".SESS_ID."'";
		...
		$sql = "UPDATE ecs_cart SET ".$set." WHERE session_id='".SESS_ID."' AND rec_id IN (".implode(',',$id).")";
		$db->query($sql);
...
/*------------------------------------------------------ */
//-- 更新购物车商品属性
//--Modify By SouL_Z 2014-8-4 23:47
/*------------------------------------------------------ */

elseif ($_REQUEST['step'] == 'update_attr')
{
    include('includes/cls_json.php');
    $json = new JSON;
    $id = $_POST['id'];

    $spec[] = intval($_POST['spec'])?intval($_POST['spec']) : 0;
    if(!empty($id)){
         if ($GLOBALS['_CFG']['use_storage'] == 1){
            $sql = "SELECT goods_id,goods_attr_id,goods_number FROM  " .$GLOBALS['ecs']->table('cart')." WHERE rec_id='".$id."' AND session_id='".SESS_ID."'";
            $cart = $GLOBALS['db']->getRow($sql);
            if(!empty($cart)){             
                //$goods = get_exchange_goods_info($cart['goods_id']);
                /* 如果商品有规格则取规格商品信息 配件除外 */
                ...
                if (is_spec($spec) && !empty($prod))
                {
                    ...
                    {
                        /* 取规格的货品库存 */
                        if ($cart['goods_number']> $product_info['product_number'])
                        {
                            $res = array('code'=>'-3','message' => '该规格库存不足','data' =>$cart['goods_number']);
                            die($json->encode($res));
                        }
                    }
                }       
        ...
        $set_arr[] = "goods_attr = '".get_goods_attr_info($spec)."'";
        $set_arr[] = "goods_attr_id = '".implode(',',$spec)."'";
        //$set_arr[] = "goods_price = '".get_final_price($cart['goods_id'], $cart['goods_number'], true, $spec)."'";
        $set = implode(',',$set_arr);

        $sql = "UPDATE ecs_cart SET ".$set." WHERE session_id='".SESS_ID."' AND rec_id = '".$id."'";
        $db->query($sql);
        //$data = get_update_cart();
        $res = array('code'=>'0','message' => '更新成功','data' => $data);
    }else{
        $res = array('code'=>'-1','message' => '更新失败');
    }
    die($json->encode($res));
...
elseif ($_REQUEST['step'] == 'change_default_address'){
    include_once('includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => 1,'content'=>'');
    $address_id = intval($_GET['address_id']);	
    $sql = "SELECT count(*) FROM  " .$GLOBALS['ecs']->table('user_address')." WHERE address_id='".$address_id."' AND user_id='".$_SESSION['user_id']."'";
    if($GLOBALS['db']->getOne($sql)){
        ...
        $smarty->assign('consignee_list', $consignee_list);
        $result['error'] = 0;
        $result['content'] = $smarty->fetch('library/checkout_address.lbi');
        ...
        $result['confirm_address'] = $smarty->fetch('library/confirm_address.lbi');
    }
...
//-- 改变收货地址
/*------------------------------------------------------ */
elseif ($_REQUEST['step'] == 'change_address'){
    include_once('includes/cls_json.php');
    $json = new JSON;
    $result = array('error' => 1,'content'=>'');
    $address_id = intval($_GET['address_id']);  
    $sql = "SELECT * FROM  " .$GLOBALS['ecs']->table('user_address')." WHERE address_id='".$address_id."' AND user_id='".$_SESSION['user_id']."'";
...
elseif ($_REQUEST['step'] == 'batch_drop_goods')
{
    $rec_ids = $_POST['ids'];
    if(!empty($rec_ids)){
        if(!is_array($rec_ids)){
            $temp = explode(',',$rec_ids);
            $rec_ids = array();
            $rec_ids = $temp;
            unset($temp);
        }
        ...
        include('includes/cls_json.php');
        $json = new JSON;
        $res = array('code'=>'777','message' => '删除成功');
        die($json->encode($res));
    }
}
/* 把优惠活动加入购物车 */
...
else
{
	...
	if($cart_goods['total']['integral']!=0){
    $smarty->assign('shopping_money',         sprintf($_LANG['shopping_money'], $cart_goods['total']['integral'].$_CFG['integral_name'].'+'.$cart_goods['total']['goods_price'])); //2014-04-28 Update by Jole
	}else{
    $smarty->assign('shopping_money',         sprintf($_LANG['shopping_money'], $cart_goods['total']['exchange_integral'].$_CFG['integral_name'].'+'.$cart_goods['total']['goods_price'])); //2014-04-28 Update by Jole
    ...
    $sql = "SELECT goods_id " .
            ...
            " WHERE session_id = '" . SESS_ID . "' " .
              "AND ( rec_type = '" . CART_GENERAL_GOODS . "' " .
            "or  rec_type = '" . CART_EXCHANGE_GOODS . "') " .
            "AND is_gift = 0 " .
    ...
    $smarty->assign('residue_integral', $residue_integral);
	if($residue_integral > 0){
		//剩余积分可兑换列表
		$children = get_children(0);
		$smarty->assign('surpluslist', exchange_get_goods($children, 0, $residue_integral, '', 5, 1, 'goods_id', 'desc'));
	}
	//你可能会喜欢的宝贝 使用推荐商品
	$smarty->assign('maybe_like', get_recommend_goods('best','',5));
}
...
function flow_cart_stock($arr)
{
    foreach ($arr AS $key => $val)
    {
        ...
        $sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart').
               " WHERE rec_id='$key' AND session_id='" . SESS_ID . "' AND now_buy = 1";
        $goods = $GLOBALS['db']->getRow($sql);

        $sql = "SELECT g.goods_name, g.goods_number, c.product_id ".
                "FROM " .$GLOBALS['ecs']->table('goods'). " AS g, ".
                    $GLOBALS['ecs']->table('cart'). " AS c ".
                "WHERE g.goods_id = c.goods_id AND c.rec_id = '$key' AND c.now_buy = 1";
        $row = $GLOBALS['db']->getRow($sql);
...
function get_update_cart(){
    ...
    " WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' AND c.now_buy = 1" .
    ...
    $canuse = @get_can_use_winning($_SESSION['user_name']);
    $data['residue_integral'] = max(0,$canuse - $data['buy_integral_amount']);
    return $data;
}
?>