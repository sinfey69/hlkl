...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_name, u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points,u.mobile_province,u.mobile_supplier,'.  //2014-04-25 Add by Jole
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
		...
		$province = $row['mobile_province'];
		$supplier = $row['mobile_supplier'];
		if(empty($province) || empty($supplier)){
			$mobile_area = checkMobilePlace($row['user_name']);  //2014-04-25 Add by Jole
			$province = $mobile_area['province'];
			$supplier = $mobile_area['supplier'];
			
			$sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . " SET mobile_province = '$province',mobile_supplier='$supplier' WHERE user_id = '$_SESSION[user_id]'";
			$GLOBALS['db']->query($sql);
		}
		$_SESSION['province'] = $province;    //2014-04-25 Add by Jole
        $_SESSION['supplier'] = $supplier;    //2014-04-25 Add by Jole
...
function get_user_info($id=0)
{
	...
	/*ccs用户积分*/
	$canuse=get_can_use_winning($user['username']);
	$winning=get_user_winning($user['username']);
	$user['jf'] = '您当前的可用积分：<font color="#BD0368">'.max(0,$canuse).'</font>分，总积分：<font color="#BD0368">'.$winning.'</font>分';
	/*修改用户积分*/
	if($user['pay_points'] != $canuse){
		$sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') .
				" SET pay_points = ".$canuse." WHERE user_name = '".$user['username']."'";
		$GLOBALS['db']->query($sql);
		$user['pay_points']=$canuse;
	}

	//if(check_ccs($user['username'])){
	  // $user['jf'] = '您当前的可用积分：<font color="#BD0368">'.get_can_use_winning($user['username']).'</font>分';
	//}
...
function assign_ur_here($cat = 0, $str = '')
{
	...
	$page_title = $GLOBALS['_CFG']['shop_title']; //2014-04-13 Update by Jole
    ...
    if ($filename != 'index')
    {
        ...
        else
        {
            ...
            /* 其他的在这里补充 */
			elseif ('draw' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['exchange_draw'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="draw.php">' .
                                $GLOBALS['_LANG']['exchange_draw'] . '</a>';
            }
			/* 其他的在这里补充 */
			elseif ('binfen' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['exchange_binfen'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="binfen.php">' .
                                $GLOBALS['_LANG']['exchange_binfen'] . '</a>';
            }
        }
    }
 ...
 * @return  int         数量
 */
function get_library_number($library, $template = null,$def_num = 3)
{
    global $page_libs;
    ...
    else
    {
        ...
        $num = isset($static_page_libs[$template][$lib]) ? $static_page_libs[$template][$lib] :  $def_num;
    }
