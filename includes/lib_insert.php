...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price, integral FROM ' . $GLOBALS['ecs']->table('goods') .
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<ul class="clearfix"><li class="goodsimg"><a href="'.$goods['url'].'" target="_blank"><img src="'.$goods['goods_thumb'].'" alt="'.$goods['goods_name'].'" class="B_blue" /></a></li><li><a href="'.$goods['url'].'" target="_blank" title="'.$goods['goods_name'].'">'.$goods['short_name'].'</a><br />'.'<font class="f1">'.$row['integral'].$GLOBALS['_CFG']['integral_name'].'+'.$goods['shop_price'].'</font><br /></li></ul>';
        }
...
function insert_bought_notes($arr)
{
    ...
    /* 商品购买记录 */
    $sql = 'SELECT u.user_name, og.goods_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
