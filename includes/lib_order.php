...
function order_fee($order, $goods, $consignee)
{
    ...
    foreach ($goods AS $val)
    {
        ...
        $total['jifen']        += $val['integral'] * $val['goods_number'];   //2014-04-28 Add by Jole
        $total['goods_price']  += $val['goods_price'] * $val['goods_number'];
        $total['market_price'] += $val['market_price'] * $val['goods_number'];
        $total['goods_count'] += $val['goods_number'];	//商品总件数 Add by SouL_Z 2014-8-1 11:01
    }
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE  `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND now_buy = 1 AND `is_shipping` = 0";
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        $sql = 'SELECT SUM(eg.exchange_integral*c.goods_number) '.
               ...
               '  AND c.now_buy = 1 AND c.is_gift = 0 AND c.goods_id > 0 ';// .
               //'GROUP BY eg.goods_id';
        $exchange_integral = $GLOBALS['db']->getOne($sql);
		//var_dump($exchange_integral);
...
/**
 * 取得购物车商品
 * @param   int     $type   类型：默认积分商城商品
 * @param   int     $now_buy   是否只筛选立即购买的商品
 * @return  array   购物车商品数组
 */
function cart_goods($type = CART_EXCHANGE_GOODS,$now_buy = 1,$is_wap = 0)
{
	$where_buy = ($now_buy) ? ' AND c.now_buy = 1 ' : '';
	$where_user = ($is_wap) ? "AND user_id = '" . $_SESSION['user_id'] . "'" : " AND session_id = '" . SESS_ID . "'";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
            "c.market_price, eg.goods_price, eg.exchange_integral, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " . //2014-04-28 Update by Jole
            "eg.goods_price * c.goods_number AS subtotal, " .
            "eg.exchange_integral * c.goods_number AS jfsubtotal " .  //2014-04-28 Add by Jole
            "FROM " . $GLOBALS['ecs']->table('cart') .' as c,' . $GLOBALS['ecs']->table('exchange_goods') . 'AS eg '.
            " WHERE c.goods_id = eg.goods_id " .
            "AND rec_type = '$type'".$where_buy.$where_user;
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['formated_jfsubtotal']   = $value['jfsubtotal']; //2014-04-28 Add by Jole
...
function cart_integral_goods($type = CART_EXCHANGE_GOODS,$now_buy = 1)
{

	$where_buy = ($now_buy) ? ' AND c.now_buy = 1 ' : '';
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
            "c.market_price, eg.goods_price, c.integral,c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, eg.exchange_integral, " . //2014-04-28 Update by Jole
            "eg.goods_price * c.goods_number AS subtotal, " .
            "eg.exchange_integral * c.goods_number AS jfsubtotal " .  //2014-04-28 Add by Jole
			...
			'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c,' . $GLOBALS['ecs']->table('exchange_goods') . 'AS eg '.
			 "WHERE c.goods_id = eg.goods_id AND c.session_id= '" . SESS_ID . "' " .
			 "  AND c.rec_type = '" . $type . "' " .
			 '  AND c.is_gift = 0 AND c.goods_id > 0 '.$where_buy; 
//            echo $sql;
    ...
    foreach ($arr as $key => $value)
    {
		$goods_thumb = $GLOBALS['db']->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['ecs']->table('goods') . " WHERE `goods_id`='{$value['goods_id']}'");//增加结算页商品图片 
 ...
 * @return  float   购物车总金额
 */
function cart_amount($include_gift = true, $type = CART_EXCHANGE_GOODS,$now_buy = 1)
{
	
	$where_buy = ($now_buy) ? ' AND now_buy = 1 ' : '';
    $sql = "SELECT SUM(goods_price * goods_number) " .
            " FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' " .
            "AND rec_type = '$type' ".$where_buy;
...
/*
	...
	*/
    ...
    $parent = array(
		...
		'integral'      => $goods['exchange_integral'], //2014-04-13 Add by Jole
        'goods_attr'    => addslashes($goods_attr),
        'goods_attr_id' => $goods_attr_id,
        'is_real'       => $goods['is_real'],
        'extension_code'=> 'exchange_goods',//$goods['extension_code'],$order['extension_code'] == 'exchange_goods'
        'is_gift'       => 0,
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                //$goods_price = get_final_price($goods_id, $num, true, $spec);
				$goods_price = max($goods['goods_price'], 0);
        ...
        {
            //$goods_price = get_final_price($goods_id, $num, true, $spec);
            $parent['goods_price']  = max($goods['goods_price'], 0);
 ...
 * @param   int     $now_buy   是否只筛选立即购买的商品
 */
function clear_cart($type = CART_EXCHANGE_GOODS , $now_buy = 1)
{
	$where_buy = ($now_buy) ? ' AND now_buy = 1 ' : '';
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type'".$where_buy;
    $GLOBALS['db']->query($sql);
}
 ...
 * @return  array
 */
function get_cart_goods($now_buy = 1)
{
    /* 初始化 */
    $goods_list = array();
    $total = array(
        ...
        'buy_goods_amount' => 0, // 立即购买金额
        'buy_integral_amount' => 0, // 立即购买积分
        'buy_goods_count' => 0, // 立即购买数量
    );
	...
	$sql = "SELECT c.*,eg.exchange_integral,eg.goods_price ,IF(c.parent_id, c.parent_id, c.goods_id) AS pid " .
		...
		" WHERE c.session_id = '" . SESS_ID . "' AND c.rec_type = '" . CART_EXCHANGE_GOODS . "'" .$where_buy.
...
//echo $sql;
    ...
    {
		if($row['now_buy'] ==1){
			$total['buy_goods_count'] += $row['goods_number'];
			if($row['integral']!=0){
				$total['buy_integral_amount']     += $row['integral'] * $row['goods_number']; //2041-04-28 Add by Jole
			}else{
				$total['buy_integral_amount']     += $row['exchange_integral'] * $row['goods_number']; //2041-04-28 Add by Jole
			}
			$total['buy_goods_amount'] += $row['goods_price'] * $row['goods_number'];

		}
		if($row['integral']!=0){
			$total['integral']     += $row['integral'] * $row['goods_number']; //2041-04-28 Add by Jole
			$row['jfsubtotal']   = $row['integral'] * $row['goods_number']; //2041-04-28 Add by Jole
		}else{
			$total['integral']     += $row['exchange_integral'] * $row['goods_number']; //2041-04-28 Add by Jole
			$row['jfsubtotal']   = $row['exchange_integral'] * $row['goods_number']; //2041-04-28 Add by Jole
		}
        ...
        /* 查询规格 */
		$attr_arr = explode(',',$row['goods_attr_id']);
		$properties = get_goods_properties($row['goods_id']); //Add By SouL_Z 2014-7-30 14:55
		$row['spe'] = $properties['spe'];
		foreach($row['spe'] as $key => $spe){
			foreach($spe['values'] as $k => $v){
				if(in_array($v['id'],$attr_arr)){
					$tempArr[] = $spe['values'][$k];
					unset($spe['values'][$k]);
				}
			}
			if(!empty($tempArr)){
				//print_r($tempArr);
				$row['spe'][$key]['values'] = array_merge($tempArr,$spe['values']);
				unset($tempArr);
			}
		}
		//print_r($row['spe']);
	...
	$total['integral_amount'] = $total['integral']; //2014-04-28 Add by Jole
