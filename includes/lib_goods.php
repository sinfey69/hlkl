 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '',$def_num = 3)
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if (empty($type_array))
        {
            foreach($type2lib as $key => $data)
            {
                ...
                {
                    $num = get_library_number($data,null,$def_num);
        ...
        $sql = 'SELECT eg.exchange_integral,eg.goods_price,g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['goods_price']          = $row['goods_price'];
 ...
 * @return  array
 */
function get_cat_goods($cat_id, $num = 0)
{
    $children = get_children($cat_id);

    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot,' . 
           "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
           'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, g.integral ' .
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
           'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
           "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
           'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
	...
	$order_rule = 'ORDER BY g.sort_order'; 
    $sql .= $order_rule;
    if ($num > 0)
    {
        $sql .= ' LIMIT ' . $num;
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                        sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
        $goods[$idx]['integral']     = $row['integral'];
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
    }
