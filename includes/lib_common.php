 ...
 *
 * @access      public
 * @param       int     region_id    地区编号
 * @return      array
 */
function get_regionsName($region_id = 0)
{
...
function load_config()
{
	...
	//file_put_contents("config.txt",json_encode($data));
    if ($data === false)
    {
...
function zip_download($list){
    require_once(ROOT_PATH . 'includes/PHPExecl/PHPExcel.php');
    require_once(ROOT_PATH . 'includes/PHPExecl/PHPExcel/Writer/Excel2007.php');
	...
	foreach($cell as $val){
		$objActSheet->getColumnDimension($val)->setWidth(25); 
		$objActSheet->getStyle($val)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objActSheet->getStyle($val.'1')->getFont()->setBold(true);
		$objActSheet->getStyle($val.'1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	}
	$Count = count($list);
    for ($i = 0; $i <= $Count; $i++) {
		$subCount = count($list[$i]);
        for ($a = 0; $a <= $subCount; $a++) {
            $v=$list[$i][$a];
            if(isset($v)){
                $h=$i+1;
                $c=$cell[$a].$h;
  				$objActSheet->setCellValueExplicit($c,$v,PHPExcel_Cell_DataType::TYPE_STRING);
            }
        }
    }
    $objWriter = new PHPExcel_Writer_Excel2007($objPHPExcel);
    $dir = local_date("YmjHis");
    if(!file_exists($dir))
    {
       mkdir($dir,0777);
    }
    $objWriter->save($dir."\data.xlsx");
    require_once(ROOT_PATH . 'includes/cls_zip.php');
    $zip = new Zip();
    $zip -> ZipAndDownload($dir);
    $zip -> deldir($dir);
    die();
}

// 手机号码归属地(返回: 如 广东移动) //2014-04-25 Add by Jole
function checkMobilePlace($mobilephone){ 
   ...
   $arr = explode("'",$content);
   $sms['province'] = conv2utf8($arr[3]);  //截取字符串
   $sms['supplier'] = conv2utf8($arr[5]);
   return $sms;
} 

// 转换字符串编码为 UTF8 //2014-04-25 Add by Jole
...
*/
function get_ccs_result($mobile){
	$res=array();
	...
	$arr=object_array($cliente->__call('ReadClerkWinning',array("parameters"=>$param)));
 ...
 * 返回true表示修改成功
 */
function update_Winning($mobile, $win,$order_sn){
	  $res=array();
	  $param = array('tUcode' => '', 'tUpsw' => '', 'tMobile' => '');
	  $param['tUcode'] = 'HOLYES88';
	  $param['tUpsw'] = 'D188BFF769EC94B540AE12B3A41FEE7C';
...
/* 判断是否CCS会员
  2014-04-25 Add by Jole */
function check_ccs($mobile){
   $check_res = array();
   $check_res = get_ccs_result($mobile);
   return $check_res[0];
}

/* 获得CCS会员的积分信息
   2014-04-25 Add by Jole */
function get_jf_info($mobile){
   $arr = array();
   $res= array();
   $arr = get_ccs_result($mobile);
...
/* 获得CCS的店员总积分
  2014-04-25 Add by Jole */
function get_user_winning($mobile){
  $arr = array();
  $arr = get_jf_info($mobile);
  return $arr['Winning'];
}

/* 获得CCS的店员总兑换分数
  2014-04-25 Add by Jole */
function get_user_ConvertWinning($mobile){
  $arr = array();
  $arr = get_jf_info($mobile);
...
/* 获得CCS会员的可用积分
   2014-04-25 Add by Jole */
function get_can_use_Winning($mobile){
	$arr = array();
	$arr = get_jf_info($mobile);
