...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    if ($log_id > 0)
    {
		...
		//file_put_contents("bill/lastsql.txt",$sql."\r\n".PAY_ORDER,FILE_APPEND);

        if ($pay_log && $pay_log['is_paid'] == 0)
        {
			$url='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
			file_put_contents("bill/respond_url.txt",$url."\r\n",FILE_APPEND);
            ...
            {
				//echo $pay_log['order_type']; 
                /* 取得订单信息 */
                /*$sql = 'SELECT order_id, user_id, order_sn, consignee, address, tel, shipping_id, extension_code, extension_id, goods_amount,integral ' .
                        'FROM ' . $GLOBALS['ecs']->table('order_info') .
                       " WHERE order_id = '$pay_log[order_id]'";*/

				 $sql = 'SELECT o.is_deduct,o.order_id, o.user_id, o.order_sn, o.consignee, o.address, o.tel, o.shipping_id, o.extension_code, o.extension_id, o.goods_amount,o.integral,u.user_name ' .
						"FROM " . $GLOBALS['ecs']->table('order_info') . " AS o, " . $GLOBALS['ecs']->table('users') . " AS u " .
                       " WHERE o.order_id = '$pay_log[order_id]' and u.user_id=o.user_id";
				...
				if($order['integral']>0 && $order['is_deduct'] == 0 ){
					$score = get_can_use_winning($order['user_name']);
					if(  $score >= $order['integral']){
						$data=update_Winning($order['user_name'], $order['integral'] ,$order_sn);
                        if($data == 'true'){
							$sql = "UPDATE ".$GLOBALS['ecs']->table('order_info')." SET is_deduct=1 WHERE order_id=".$order_id;
                            $GLOBALS['db']->query($sql);
                        }
						$log=$data."状态 ".date('Y-m-d H:i:s',time())."用户：".$order['user_name']." 兑换：".$order['integral']."\r\n";
						file_put_contents("bill/convertlog.txt",$log."\r\n",FILE_APPEND);
					}
				}else{
					$log="-----状态 ".date('Y-m-d H:i:s',time())."用户：".$order['user_name']." 兑换：".$order['integral']."\r\n";
						file_put_contents("bill/convertlog.txt",$log."\r\n",FILE_APPEND);
				}
