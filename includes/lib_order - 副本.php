...
/*获取商品所需要积分*/
function get_exchange_integral(){
	$sql = 'SELECT eg.goods_id,eg.exchange_integral '.
		   'FROM ' . $GLOBALS['ecs']->table('cart') . ' AS c,' . $GLOBALS['ecs']->table('exchange_goods') . 'AS eg '.
		   "WHERE c.goods_id = eg.goods_id AND c.session_id= '" . SESS_ID . "' " .
		   "  AND c.rec_type = '" . CART_EXCHANGE_GOODS . "' " .
		   '  AND c.is_gift = 0 AND c.goods_id > 0 '; 
			   
    $exchange_integral = $GLOBALS['db']->getAll($sql);	
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            "market_price, goods_price, integral, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, " . //2014-04-28 Update by Jole
            "goods_price * goods_number AS subtotal, " .
            "integral * goods_number AS jfsubtotal " .  //2014-04-28 Add by Jole
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $parent = array(
		...
		'integral'      => $goods['integral'], //2014-04-13 Add by Jole
