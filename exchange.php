...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    $cache_id = $goods_id . '-' . $_SESSION['user_rank'] . '-' . $_CFG['lang'] . '-' . $residue_integral .'-exchange';
...
elseif ($_REQUEST['act'] == 'buy')
{
	...
	/*判断已下单未支付的订单 的总兑换积分*/
	$record_count = $db->getOne("SELECT sum(integral) FROM " .$ecs->table('order_info'). " WHERE order_status=0 and  user_id = '$user_id'");
    ...
    {
        $attr_list[] = $row['attr_name'] . ': ' . $row['attr_value'].': ' . $row['attr_price'];
    }
    //var_dump($attr_list);
    ...
    $cart = array(
        ...
        'extension_code' => 'exchange_goods',//addslashes($goods['extension_code']),
        'parent_id'      => 0,
...
function get_exchange_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, eg.exchange_integral, eg.is_exchange,eg.goods_price ' .
            ...
            "WHERE eg.is_exchange = 1 AND g.goods_id = '$goods_id' AND g.is_delete = 0 " .
            'GROUP BY g.goods_id';
