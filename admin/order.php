...
file_put_contents("../bill/lastsql.txt",$delivery_stock_sql."\r\n",FILE_APPEND);
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $value)
        {
            ...
            if ($value['is_real'] != 0)
            {
				...
				file_put_contents("../bill/lastsql.txt",$minus_stock_sql."\r\n",FILE_APPEND);
                $GLOBALS['db']->query($minus_stock_sql, 'SILENT');
            }
        }
    }
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['ship']))
    {
		...
		/*
		#检查是否已扣积分#
		if($order['is_deduct'] == 0){
			if(@update_Winning1($order['user_name'], $order_info['integral']) == 'true'){
				$sql = "UPDATE ".$ecs->table('order_info')." SET is_deduct=1 WHERE order_id=".$order_info['order_id'];
				$db->query($sql);
			}else{
    ...
    elseif ('split' == $operation)
    {
        ...
        if ($delivery_id)
        {
            ...
            if (!empty($goods_list))
            {
                ...
                {
                    // 商品（实货）（虚货）
                    if (empty($value['extension_code']) || $value['extension_code'] == 'exchange_goods' || $value['extension_code'] == 'virtual_card')
                    {
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.pay_name, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.province,o.city,o.district,o.address, o.email, o.tel, o.extension_code, o.extension_id, " .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,u.mobile_supplier,u.mobile_province ".
    ...
    {
		$row[$key]['province'] = get_regionsName($value['province']);
		$row[$key]['city'] = get_regionsName($value['city']);
		$row[$key]['district'] = get_regionsName($value['district']);
		...
		//获取下单时间段
		$row[$key]['time_quantum'] = get_time_quantum(local_date('G',$value['add_time']));
    }
