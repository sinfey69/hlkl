...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,visit_count,mobile_supplier,mobile_province,last_login ".
	...
	foreach($user_list as $i=>$val){
        $user_list[$i]['reg_time'] = local_date($GLOBALS['_CFG']['date_format'], $val['reg_time']);
        $user_list[$i]['last_login'] = !empty($val['last_login']) ? local_date('Y-m-d H:i:s', $val['last_login']) : '-';
        $user_list[$i]['winning'] = get_user_winning($val['user_name']);
        $user_list[$i]['canuse'] = 	get_can_use_winning($val['user_name']);

		$sql = "SELECT re.content "."FROM " . $GLOBALS['ecs']->table('reg_extend_info') . " as re LEFT JOIN " . 
			$GLOBALS['ecs']->table('reg_fields') . " as rf ON rf.id = re.reg_field_id".
			" WHERE rf.reg_field_name='店名' AND re.user_id=".$val['user_id'];
		$shop_name = $GLOBALS['db']->getOne($sql);
        $user_list[$i]['shop_name'] = $shop_name;
	}
