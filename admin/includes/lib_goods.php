...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
	...
	foreach($row as $key=>$value){
		$sql="select exchange_integral,goods_price as exchange_price from ".$GLOBALS['ecs']->table('exchange_goods') ." where goods_id=".$value['goods_id'];
		$exchage_goods = $GLOBALS['db']->getRow($sql);
		$row[$key]['exchange_integral'] = $exchage_goods['exchange_integral'];
		$row[$key]['exchange_price'] = $exchage_goods['exchange_price'];
	}
